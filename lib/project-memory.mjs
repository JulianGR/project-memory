...
async function appendEvent(directory) {
  ...
  for (;;) {
    const ordinal = (await readdir(directory)).reduce((highest, file) => {
      const match = /^(\d+)\.event$/.exec(file)
      const value = match ? Number(match[1]) : 0
      return Number.isSafeInteger(value) && value > highest ? value : highest
    }, 0) + 1
    try {
      await writeFile(join(directory, `${ordinal}.event`), '', { encoding: 'utf8', flag: 'wx' })
      return ordinal
    } catch (error) {
...
export async function recordPrompt({ host, event, stateRoot = stateRootFor(), interval = resolveInterval() }) {
  ...
  const count = await appendEvent(events)
