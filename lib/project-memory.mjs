...
export function reviewInstruction(root) {
  ...
  return `Project memory: read ${path} and follow its maintenance policy. Before finishing this turn, review whether it changed durable project knowledge, including non-code work and accepted decisions. Update only the managed state section when relevant information changed; otherwise leave the file untouched. Consolidate by topic, not by turn or commit. Preserve still-valid decisions and their rationale regardless of age. Remove or replace facts only with evidence of supersession or lost applicability. Do not edit unrelated instructions or create another memory file. If this review is already complete and nothing new changed, finish without repeating work.`
}

export function formatContext(host, hookEventName, message) {
  return host === 'kimi' ? message : JSON.stringify({ hookSpecificOutput: { hookEventName, additionalContext: message } })
}
